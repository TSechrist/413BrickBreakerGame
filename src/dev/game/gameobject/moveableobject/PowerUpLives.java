...
public class PowerUpLives extends MoveableObject {

    private BufferedImage img = Assets.poweruplives;
    private int SPEED = 5;

    public PowerUpLives(Handler handler, float x, float y, int width, int height) {
    ...
    public void tick() {
        ...
        if(checkObjectCollisions(0, SPEED)) {
            ...
            {
                handler.getMap().getObjectManager().getTank1().setLives(handler.getMap().getObjectManager().getTank1().getLives() + 1);
                handler.getMap().getObjectManager().removeObject(this);
            }
        }
    }
